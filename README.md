...
- 提供包含私钥和 `login.bat` 的 ZIP 下载，不生成 `.ps1` 文件
...
管理页面提供两种 Windows 登录方式：复制带横向滚动条的 PowerShell 单行命令，或下载只包含私钥和 `login.bat` 的一键登录包。页面不再单独显示 BAT/CMD 命令，两种方式都会自动安装缺失的 `cloudflared`。
